...
export async function generateInvoiceNumber(prisma: PrismaClient, invoiceDate: Date) {
  const month = invoiceDate.toISOString().slice(0, 7).replace("-", "");
  const start = new Date(invoiceDate.getFullYear(), invoiceDate.getMonth(), 1);
